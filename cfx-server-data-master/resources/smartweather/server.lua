...
local admins = {
	"steam:1100001021627ac", --skrub
	"steam:1100001042ffec7", --kallam
	"steam:110000100084851", --heather
	"steam:110000104ef60e8", --rockford
	"steam:110000119bea6aa", --starmanis
	"steam:1100001066b0c23", --zachary
	"steam:1100001066ceebd", --adam
	"steam:110000114400dbd", --glendale
	"steam:1100001010c1bfb", --genovah
	"steam:11000010221fde4", --alex
}
...
AddEventHandler("smartweather:toggleWeather",function(from)
	...
	local message = "Weather has been "..text.." by ^5"..by
	TriggerClientEvent("chatMessage", from, "Weather",{0,0,0},message)
end)
...
function handleAdminCheck(from)
	if( adminOnlyPlugin and not(isAdmin(getIdentifier(from, "steam"))) )then
		TriggerClientEvent('chatMessage', from, "", {200,0,0} , "You must be an admin to use this command.")
		return false
	end
	return true
end
...
AddEventHandler('chatMessage', function(from,name,message)
	if(string.sub(message,1,1) == "/") then
		...
		if(cmd == "/setweather")then
			...
			if(wtype == nil)then
				TriggerClientEvent('chatMessage', from, "", {200,0,0} , "Usage: /setweather CLEAR")
				return
			end

			if(weatherTree[wtype] == nil)then
				TriggerClientEvent('chatMessage', from, "", {200,0,0} , "Invalid weather type, valid weather types below")
...
function weatherCheck()
	...
	if(SmartWeatherEnabled)then
		...
		print("Weather is currently disabled")
	end
