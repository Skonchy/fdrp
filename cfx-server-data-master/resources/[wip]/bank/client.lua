...
local robbery = {
	...
	{["x"] = -116.565, ["y"] = 6457.380, ["z"] = 31.454}
}

local guards = {
    -- Blaine County Savings bank
	{x = -105.387, y = 6470.396, z = 31.627, heading= 139.493}
}
...
function AddGuards(x, y, z, heading)
	RequestModel(GetHashKey("s_m_m_security_01"))
	while not HasModelLoaded(GetHashKey("s_m_m_security_01")) do
		Wait(0)
	end

	pedguard = CreatePed(5, GetHashKey("s_m_m_security_01"), x, y, z-1.0001, heading, true, false)
	GiveWeaponToPed(pedguard, GetHashKey("WEAPON_PISTOL"), 80, true, true)

	SetPedRelationshipGroupHash(pedguard, GetHashKey("SECURITY_GUARD"))
	SetEntityAsMissionEntity(pedguard, true, true)	
	SetModelAsNoLongerNeeded(GetHashKey("s_m_m_security_01"))

	blipTest = AddBlipForEntity(pedguard)
	SetBlipSprite(blipTest, 1)
	SetBlipColour(blipTest, 3)
end

Citizen.CreateThread(function()

	for k,v in pairs(guards) do
		Citizen.Trace('Loading guards\n')
		AddGuards(v.x, v.y, v.z, v.heading)
	end
...
Citizen.CreateThread(function()
	...
	PrepareAlarm("PALETO_BAY_SCORE_ALARM")
	
	while true do
		Wait(0)

		if IsAlarmPlaying("PALETO_BAY_SCORE_ALARM") then
			SetPlayerWantedLevel(PlayerId(), 3, true)
			SetPlayerWantedLevelNow(PlayerId(), true)
		end

		if DoesEntityExist(GetClosestObjectOfType(-104.6049, 6473.444, 31.79532, 20.0, GetHashKey("v_ilev_cbankvauldoor01"), 0, 1, 0)) then
			vaultDOOR = GetClosestObjectOfType(-104.6049, 6473.444, 31.79532, 20.0, GetHashKey("v_ilev_cbankvauldoor01"), 0, 0, 0)
