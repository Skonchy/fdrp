...
AddEventHandler('esx_phone:startCall', function(number)
	...
	if foundPlayer ~= false then
		...
		TriggerClientEvent('esx_phone:endCall', _source, 'This number is unavailable')
	end
end)
		...
		if result[1] ~= nil then
			...
			if phoneNumber == xPlayer.get('phoneNumber') then
				...
				if hasAlreadyAdded then
					TriggerClientEvent('esx:showNotification', _source, 'This contact is already in your directory')
				else
			...
			TriggerClientEvent('esx:showNotification', source, 'This number is non-existent')
		end
...
AddEventHandler('esx_phone:bankTransfer', function(target, amount)
	...
	if amount > 0 and amount <= xPlayer.getAccount('bank').money then
		...
		TriggerClientEvent('esx:showNotification', _source, 'You wired ~g~$' .. amount .. '~s~ to ' .. targetXPlayer.firtsname .. targetXPlayer.lastname)
	else
