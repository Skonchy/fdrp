	...
	while true do
		...
		if GetLastInputMethod(2) then -- Only executes the following if the Input came from a Keyboard or Mouse
			if IsControlJustReleased(1, 11) then
				if not menu then
					if IsPedInAnyPoliceVehicle(GetPlayerPed(-1), 0) and (GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), 0), -1) == GetPlayerPed(-1)) then
						menu = true
					else
						drawNotification("~r~You have to be the driver of a law enforcement vehicle to use this Menu.")
					end
				elseif menu then
					menu = false
				end
			...
			elseif not (IsPedInAnyPoliceVehicle(GetPlayerPed(-1), 0) and (GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), 0), -1) == GetPlayerPed(-1))) then
				menu = false
				trailer = false
				bool = true
				ExtraCount = 0
