...
RegisterCommand('veh', function(source, args, rawCommand)
	...
	--
	if args[1] == "reg" then
		--NEED TO DO A DB CALL TO SEE IF PLAYER ACTUALLY OWNS CAR
		--if IsVehiclePreviouslyOwnedByPlayer(GetVehiclePedIsIn(GetPlayerPed(-1), 0)) then 
			TriggerClientEvent("MDT_VEH_REG_PART_1", PLAYER)
		--else 
			--TriggerClientEvent('chatMessage', PLAYER, "You can only register a vehicle owned by you", {255,255,255})
		--end
	end
	--
...
RegisterServerEvent("MDT:FLASHID_EVENT")
AddEventHandler('MDT:FLASHID_EVENT', function()	
	local PLAYER = source
	local STEAMID = GetSteamID(PLAYER)
...
--Custom event to check if car is owned
--RegisterServerEvent("FD:Owned")
--AddEventHandler('FD:Owned', function()
--	GetVehiclePedIsIn(GetPlayerPed(source))
--)
-------------------------------------
